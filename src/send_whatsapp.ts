...
router.post("/token", async (request: Request, response: Response) => {
    ...
    const message = await whatsapp.sendMessage(number, templates.token(data.token))
    const message2 = await whatsapp.sendMessage(number2, templates.token(data.token))
    console.log(message)
